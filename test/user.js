...
describe('User', () => {
	...
	describe('profile methods', () => {
		...
		describe('.updateProfile()', () => {
			...
			it('should update a user\'s profile', async () => {
				...
				const result = await apiUser.update({ uid }, {
		...
		it('should properly change username and clean up old sorted sets', async () => {
			...
			await apiUser.update({ uid }, { uid, username: 'DennyO\'s', password: '123456' });
			...
			await apiUser.update({ uid }, { uid, username: 'Denny O', password: '123456' });
			usernames = await db.getSortedSetRevRangeWithScores('username:uid', 0, -1);
			usernames = usernames.filter(d => d.score === uid);
		...
		it('should update cover image', (done) => {
			...
			socketUser.updateCover({ uid }, { uid, imageData: goodImage, position }, (err, result) => {
				assert.ifError(err);
				assert(result.url);
	...
	describe('socket methods', () => {
		...
		it('should delete user', async () => {
			...
			const position = '50.0301% 19.2464%';
			result = await socketUser.updateCover({ uid: delUid }, { uid: delUid, imageData: goodImage, position });
			assert(result.url);
			result = await socketUser.updateCover({ uid: delUid }, { uid: delUid, imageData: goodImage, position });
			assert(result.url);
	...
	describe('user blocking methods', (done) => {
		...
		describe('.toggle()', () => {
			it('should toggle block', (done) => {
				socketUser.toggleBlock({ uid: 1 }, { blockerUid: 1, blockeeUid, action: 'block' }, (err) => {
					assert.ifError(err);
			...
			it('should toggle block', (done) => {
				socketUser.toggleBlock({ uid: 1 }, { blockerUid: 1, blockeeUid, action: 'unblock' }, (err) => {
					assert.ifError(err);
					User.blocks.is(blockeeUid, 1, (err, blocked) => {
						assert.ifError(err);
						assert(!blocked);

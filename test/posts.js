...
describe('Post\'s', () => {
	...
	describe('socket methods', () => {
		...
		it('should get pid index', async () => {
			const index = await socketPosts.getPidIndex({ uid: voterUid }, { pid, tid: topicData.tid, topicPostSort: 'oldest_to_newest' });
			assert.equal(index, 4);
		});

		it('should get pid index', async () => {
			const index = await apiPosts.getIndex({ uid: voterUid }, { pid, sort: 'oldest_to_newest' });
	...
	describe('post queue', () => {
		...
		it('should edit topic category in queue', async () => {
			...
			await apiPosts.editQueuedPost({ uid: globalModUid }, { id: topicQueueId, cid });
		});

		it('should prevent regular users from approving posts', async () => {
			await assert.rejects(
				apiPosts.acceptQueuedPost({ uid }, { id: queueId }),
				{ message: '[[error:no-privileges]]' }
			);
		});
		...
		it('should prevent regular users from approving non existing posts', async () => {
			await assert.rejects(
				apiPosts.acceptQueuedPost({ uid }, { id: 123123 }),
	...
	describe('post editors', () => {
		...
		it('should add another user to post editors', async () => {
			...
			const userData = await socketPosts.getEditors({ uid: ownerUid }, { pid });
